...
const RegisterForm = ({ error, setError, handleSubmit, isLoading }: RegisterFormProps) => {
  ...
  const onNext1 = async (values: typeof initialValues, helpers: FormikHelpers<typeof initialValues>) => {
    setError('');
    try {
  ...
  const onNext2 = async (values: typeof initialValues, helpers: FormikHelpers<typeof initialValues>) => {
    setError('');
    try {
  ...
  const onFinish = async (values: typeof initialValues, setErrors: any, setTouched: any) => {
    setError('');
    try {
              ...
              <Stack spacing={2} alignItems="center">
                <Box sx={{ width: '100%', maxWidth: 440 }}>
                  ...
                  <Stack spacing={1.5} sx={{ pt: 2 }}>
                    <Button type="button" variant="contained" fullWidth size="large" sx={BIG_BUTTON_SX} disabled={isLoading} onClick={() => onNext1(values, { setErrors, setTouched } as any)}>
            ...
            {step === 2 && (
              <Stack spacing={2} alignItems="center">
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ pt: 1, width: '100%' }}>
                  ...
                  <Button type="button" variant="contained" fullWidth size="large" sx={BIG_BUTTON_SX} disabled={isLoading} onClick={() => onNext2(values, { setErrors, setTouched } as any)}>Continue</Button>
                </Stack>
              </Stack>
            )}
              ...
              <Stack spacing={2} sx={{ overflow: 'hidden' }}>
                <Box sx={{ width: '100%', mt: 1 }}>
                  <Stack direction={{ xs: 'column', lg: 'row' }} spacing={1.25} alignItems={{ xs: 'stretch', lg: 'center' }} justifyContent="center" sx={{ width: '100%' }}>
                    ...
                    <Button size="medium" variant="outlined" startIcon={<VolumeUpIcon />} onClick={() => previewSelection({ tone: values.tone, voice: values.voice, firstName: values.firstName })} disabled={!values.voice || !values.tone || isLoading} sx={{ px: 3, minWidth: { md: 160 }, flexShrink: 0 }}>
                      Listen
                    </Button>
                  </Stack>
                </Box>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                  ...
                  <Button type="button" variant="contained" fullWidth size="large" sx={BIG_BUTTON_SX} onClick={() => onFinish(values, setErrors, setTouched)} disabled={isLoading}>
